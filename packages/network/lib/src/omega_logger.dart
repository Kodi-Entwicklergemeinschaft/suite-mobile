...
class OmegaDioLogger extends Interceptor {
  ...
  /// List of location list API endpoints to hide completely
  static const List<String> _locationListEndpoints = [
    '/user/countries',
    '/user/states',
    '/user/cities',
  ];

  /// Checks if URL is a location list API
  bool _isLocationListApi(String url) {
    return _locationListEndpoints.any((endpoint) => url.contains(endpoint));
  }
  ...
  void _requestRepresentation(RequestOptions options) {
    ...
    // Hide location list API requests
    if (_isLocationListApi(uri)) {
      _printBoxed('Request ┃ $method', '[LOCATION LIST API HIDDEN]');
      return;
    }

    _printBoxed('Request ┃ $method', uri);
  }
  ...
  void _responseRepresentation(Response<dynamic> response) {
    ...
    // Hide location list API responses
    if (_isLocationListApi(uri)) {
      _printBoxed('Response ┃ $method ┃ ${response.statusCode}', '[LOCATION LIST API HIDDEN]');
      return;
    }

    _printBoxed('Response ┃ $method ┃ ${response.statusCode}', uri);
  }
  ...
  void _responseBodyRepresentation(Response<dynamic> response) {
    ...
    // Hide location list API response bodies
    if (_isLocationListApi(uri)) {
      _printBoxed('Response Body', '[LOCATION LIST DATA HIDDEN]');
      return;
    }
  ...
  void _cURLRepresentation(RequestOptions options) {
    ...
    // Hide cURL for location list APIs
    if (_isLocationListApi(uri)) {
      _printBoxed('cURL', '[LOCATION LIST API cURL HIDDEN]');
      return;
    }
    ...
    if (data != null) {
      if (data is FormData) {
        ...
        (data as Map).forEach((dynamic k, dynamic v) => components.add('-d "$k=$v"'));
      } else {
