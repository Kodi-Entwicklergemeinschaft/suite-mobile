...
import 'package:template_c/feat/profile/domain/usecases/edit_profile_usecase.dart';
...
class EditProfileNotifier extends Notifier<EditProfileState> {
  ...
  Future<bool> _uploadStagedImage(File file) async {
    state = state.copyWith(isUploadingImage: true, message: null);
  ...
  Future<bool> _deletePendingAvatar() async {
    ...
    final result = await _imageDeleteUsecase.call(
      ImageDeleteRequestModel(mediaUrl: currentAvatarUrl, entityType: 'user'),
    );
    ...
    result.fold(
      ...
      (_) {
        ...
        if (currentProfile != null) {
          ref
              .read(profileControllerProvider.notifier)
              .setProfileData(currentProfile.copyWith(avatarUrl: ''));
        }
        success = true;
      },
    );
  ...
  Future<void> submitForm({
    ...
    final pendingDelete = state.pendingDelete;

    state = state.copyWith(state: StateEnum.loading, message: null);
    ...
    if (stagedFile != null) {
      ...
      if (!uploaded) {
        state = state.copyWith(state: StateEnum.error);
        return;
      }
  ...
  void clearError() {
    if (state.state == StateEnum.error) {
      state = state.copyWith(
        state: StateEnum.initial, // or idle
        message: null,
      );
    }
  }
