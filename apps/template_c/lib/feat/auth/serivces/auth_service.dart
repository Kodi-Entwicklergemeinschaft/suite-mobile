...
import 'package:template_c/feat/auth/data/models/respnse_model/register_response_model.dart';
...
class AuthService {
  ...
  Future<Either<Exception, RegisterResponseModel>> register(
    ...
    if (!_isLiveMode) {
      return Right(RegisterResponseModel(success: true, message: 'Registration successful.'));
    }
  ...
  Future<Either<Exception, void>> logout() async {
    if (!_isLiveMode) return const Right(null);
  ...
  Future<void> saveTokens({
    ...
    UserRole? role,
    bool isGuest = true, UserModel? user,
  }) async {
    await Future.wait([
    ...
    if (role != null) {
      ...
      if(user!=null) {
        _sharedPreferences.saveDouble(StorageKeys.lat, user.latitude??0);
        _sharedPreferences.saveDouble(StorageKeys.long, user.longitude??0);
        _sharedPreferences.saveDouble(StorageKeys.radius, user.radius?.toDouble()??0);
        _sharedPreferences.saveString(StorageKeys.selectedLocation, user.localityName??"");
        _sharedPreferences.saveBool(StorageKeys.isOnboarded, user.onboarded??false);
        _sharedPreferences.saveString(StorageKeys.userName, user.username??"GU");
      }
  ...
  Future<Either<Exception, ForgotPasswordResponseModel>> forgotPassword(
    ...
    if (!_isLiveMode) {
      return Right(ForgotPasswordResponseModel(success: true, message: 'Password reset instructions sent.'));
    }
  ...
  Future<Either<Exception, GuestAuthResponseModel>> guestAuth(
    ...
    if (!_isLiveMode) {
      return Right(GuestAuthResponseModel(
        accessToken: '',
        refreshToken: '',
        expiresIn: 86400,
      ));
  ...
  Future<Either<Exception, GuestRegisterResponseModel>> guestRegister(
    ...
    if (!_isLiveMode) {
      return Right(GuestRegisterResponseModel(success: true, message: 'Registration successful.'));
    }
  ...
  Future<Either<Exception, ChangePasswordResponseModel>> changePassword(
    ...
    if (!_isLiveMode) {
      return Right(ChangePasswordResponseModel(success: true, message: 'Password changed successfully.'));
    }
  ...
  Future<Either<Exception, MeResponseModel>> getMe() async {
    if (!_isLiveMode) {
      return Right(MeResponseModel());
    }
    return await _apiHelper.getRequest<MeResponseModel>(
      path: ApiEndpoints.authMe,
      create: () => MeResponseModel(),
    );
  }
}
